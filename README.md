...
## Props 属性
  ...
  - 使用有效正确的 aria `role`属性值 [ARIA roles](https://www.w3.org/TR/wai-aria/#usage_intro). eslint: [`jsx-a11y/aria-role`](https://github.com/evcohen/eslint-plugin-jsx-a11y/blob/master/docs/rules/aria-role.md)
  ...
  <div />
  ```
  - 避免使用数组的index来作为属性`key`的值，推荐使用唯一ID. eslint: [`react/no-array-index-key`](https://github.com/yannickcr/eslint-plugin-react/blob/master/docs/rules/no-array-index-key.md)
  -

> 为什么？不使用稳定ID[是anti-pattern的]（https://medium.com/@robinpokorny/index-as-a-key-is-an-anti-pattern-e0349aec318），它会对性能产生负面影响并导致组件状态问题。

如果项的顺序可能更改，我们不建议使用index来作为key。
