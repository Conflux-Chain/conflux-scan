export default {
  // Header Components
  'app.header.lang.en': 'English',
  'app.header.lang.zh': '中文',

  // Footer Components
  'app.footer.title': '联系我们',
  'app.footer.address': 'Room 1002, Building B, Xueyandasha, No. 30, Shuangqing Road, Beijing, P.R. China.',

  // Navbar Components
  'app.navbar.home': '首页',
  'app.navbar.dashboard': '首页',
  'app.navbar.blocksAndTxs': '区块 & 交易',
  'app.navbar.directory': '导航',
  'app.navbar.more': '更多',

  // Directory Page
  'app.directory.exchange.title': '交易所',
  'app.directory.exchange.desc': 'Places to buy and sell cryptocurrencies',
  'app.directory.wallet.title': 'Wallet',
  'app.directory.wallet.desc': 'Used to receive and spend cryptoassets',
  'app.directory.website.title': 'Office Website',
  'app.directory.website.desc': 'Used to receive and spend cryptoassets',
  'app.directory.news.title': 'News',
  'app.directory.news.desc': 'Get close to the community',
  'app.directory.topholders.title': 'Top Token Holders',
  'app.directory.topholders.desc': 'Find the events near you',
  'app.directory.community.title': 'Community',
  'app.directory.community.desc': 'Get close to the community',
};
